...
class AudioMergerApp:
    def __init__(self, root):
        ...
        tk.Button(
            root,
            text="Chọn video GỐC (có âm thanh)",
            width=40,
            command=self.select_video_with_audio
        ).pack(pady=5)

        tk.Button(
            root,
            text="Chọn video CROP (thiếu âm thanh)",
            width=40,
            command=self.select_video_no_audio
        ).pack(pady=5)

        tk.Button(
            root,
            text="GẮN LẠI ÂM THANH",
            width=40,
            height=2,
    ...
    def merge_audio(self):
        ...
        # =============================
        folder = os.path.dirname(self.video_no_audio)
        name, ext = os.path.splitext(os.path.basename(self.video_no_audio))
        output_path = os.path.join(folder, f"{name}_withAudio{ext}")
