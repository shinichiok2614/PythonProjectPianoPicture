...
class ImageNumberMarker:
    def __init__(self, master):
        ...
        self.circle_radius = tk.IntVar(value=10)
    ...
    def build_ui(self):
        ...
        tk.Button(num_btns, text="−", width=4, command=self.decrease_number).pack(side=tk.LEFT, expand=True, fill=tk.X)
        tk.Button(num_btns, text="+", width=4, command=self.increase_number).pack(side=tk.LEFT, expand=True, fill=tk.X)
    ...
    def show_image(self):
        ...
        for x, y, num, color, r, fs in self.markers:
            ...
            sr = r / self.scale_x  # scale radius
            sfs = max(1, int(fs / self.scale_x))  # scale font size

            draw.ellipse((cx - sr, cy - sr, cx + sr, cy + sr), outline=color, width=3)
